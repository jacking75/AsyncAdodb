...
void TestInsertSP()
{
	...
	while (i > 0)
	{
		...
		pAdo->UpdateParameter<COleDateTime&>(L"@v_hhh", oleTime); // 동일 쿼리문을 여러번 실행 시킬 필요가 있을때 사용합니다. 예로 들어 여러 건수의 값을 insert 할 때 사용한다
		pAdo->UpdateParameter(L"@v_hhh", oleTime);
		pAdo->UpdateParameter(L"@v_bbb", (WCHAR*)strString.c_str(), (int)strString.size());
